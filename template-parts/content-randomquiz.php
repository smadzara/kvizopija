...
if (isset($random_posts) && $random_posts->have_posts()) {
    while ($random_posts->have_posts()) {
        $random_posts->the_post();
        echo '<label class="post-question-generator">';
        the_title();
        echo '</label>';
        
        // Hide the content by default
        echo '<div class="post-answer-generator" style="display:none;">';
        the_content();
        echo '</div>';
        
        // Add a button to show the content
    }
    wp_reset_postdata();
} else {
    echo '<div style="text-align: center; margin-top: 10px; font-size: 24px; background-color: #e7f3f1;">Da bi nešto generirao, moraš odabrati kategorije i upisati broj pitanja. (:</div>';
}
?>

<button class="show-content-button">Prikaži odgovore</button>
