...
function kvizopija_scripts() {
	...
	$style_css_mtime = file_exists( $style_css_path ) ? (int) filemtime( $style_css_path ) : 0;
	$min_css_mtime   = file_exists( $min_style_path ) ? (int) filemtime( $min_style_path ) : 0;

	/*
	 * Prefer minified CSS only when it is not older than style.css.
	 * This prevents stale style.min.css from masking recent style.css edits.
	 */
	if ( $min_css_mtime > 0 && $min_css_mtime >= $style_css_mtime ) {
		$style_src     = get_template_directory_uri() . '/style.min.css';
		$style_version = (string) $min_css_mtime;
	} elseif ( $style_css_mtime > 0 ) {
		$style_version = (string) $style_css_mtime;
	}
